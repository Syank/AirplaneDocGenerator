...
class ProjectAdministrationScreen extends React.Component {
    ...
    getInformationsContainer() {
        ...
        let container = (
            <div className="flex flex-col w-1/3 pl-3 pt-1 pb-1 h-full justify-between">
                ...
                <div className="border-t-2 pt-2 border-black border-opacity-50 flex flex-col items-center text-sm">
                    <Button text="Exportar projeto" type="confirm" onClick={this.exportProject}></Button>
                </div>
            </div>
        );
    ...
    async exportProject() {
        ...
        if (supposedSelectedPath.canceled === false) {
            let project = this.state["projectData"]["nome"];
            ...
            if (response.status === 200) {
                ...
                await window.electron.windowControll.downloadFile(base64File, pathToSave, project, ".zip");

                notification(
                    "success",
                    "Eba! 🤩",
                    "Projeto exportado! Verifique a pasta " + pathToSave + "!"
                );
            }
        }
    }
